...
function calculateExpression(expression) {
  ...
  if (typeof expression === "string") {
    // Використовуємо eval(), щоб виконати переданий код
    return eval(expression);
  } else {
    // Якщо аргумент не є рядком, повертаємо відповідне повідомлення
...
function getNumberFromString(s) {
  ...
  if (!isNaN(number)) {
    ...
    // Якщо значення не є числом, повертаємо текстове повідомлення про помилку
